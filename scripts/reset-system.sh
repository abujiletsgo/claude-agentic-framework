...
ps aux | grep -E "bun.*(apps/observability/(server|client))" | grep -v grep | awk '{print $2}' | while read PID; do
    if [ -n "$PID" ]; then
...
if [ -f "$PROJECT_ROOT/apps/observability/server/events.db-wal" ]; then
...
echo "2. Or manually: cd apps/observability/server && bun run dev"
echo "3. And: cd apps/observability/client && bun run dev"