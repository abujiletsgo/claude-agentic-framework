...
Plus **correct** placement of AAAK and mempalace as the long-term memory and context archival layer — NOT as inter-agent compression (AAAK output is opaque encoding, not human-readable).
...
**All IPC files are plain text / JSON.** No AAAK compression on IPC — everything must be human-readable and debuggable.
...
## Part C: AAAK + Mempalace — Correct Placement

### What AAAK Actually Is

AAAK is a **local Python algorithm** (`mempalace.dialect.Dialect`) that compresses text ~45x average. Key facts from production data (92 runs logged in `~/.claude/data/aaak_stats.jsonl`):

- **No LLM call** — pure Python, synchronous, <100ms
- **No network** — fully offline
- **Fail-open** — returns original text if mempalace venv unavailable
- **Output is NOT human-readable** — it's opaque encoded notation
- **Production ratios**: 4974 chars → 81 chars (61x), 5224 chars → 99 chars (53x)

### What AAAK Is Good For

| Use Case | AAAK? | Explanation |
|----------|-------|-------------|
| **Pre-compact preservation** | **YES** | Already in production. Framework re-injects on decompression. |
| **Storing sprint results in mempalace palace** | **YES** | Stored compressed → decompressed on semantic search retrieval |
| **Archiving events.jsonl to mempalace** | **YES** | Long-term storage. Original stays on disk. |
| **Session summaries for future recall** | **YES** | Cross-session memory. Retrieved/decompressed when relevant. |
| **Large codebase context blocks** | **YES** | When you need to store/retrieve large code analysis later |

### What AAAK Is NOT Good For

| Use Case | AAAK? | Explanation |
|----------|-------|-------------|
| **Lead prompts** | **NO** | Leads must READ them. AAAK output is unreadable. |
| **Inter-agent results** | **NO** | PM/orchestrator must READ them. Use TOON instead for structured data. |
| **Sprint report for TUI** | **NO** | Humans must READ the report. |
| **IPC files (status.json, events.jsonl)** | **NO** | Must be debuggable. `jq` can't parse AAAK. |
| **KG triples** | **NO** | Already tiny (subject/predicate/object). No benefit. |
| **Real-time inter-lead communication** | **NO** | All parties need to read it. |

### Mempalace Integration Points in Sprint

**Two access modes exist** — hooks use direct Python imports (automated, no LLM), Claude uses MCP tools (interactive):

#### Automated (Hook-Driven, No LLM)

| When | Hook | What Happens |
|------|------|-------------|
| Sprint lead finishes | SubagentStop | `sprint_palace_store.py` stores lead result in palace (room=`sprint_results`), AAAK-compressed for storage efficiency. Extracts decision statements → KG triples. |
| Sprint completes | Stop | `auto_memory_writer.py` writes sprint summary to `.claude/MEMORY.md` + KG triple. |
| Next sprint starts | SubagentStart | `inject_sprint_context.py` queries KG for prior decisions, injects ≤2000 chars into lead context. |
| Context compacts | PreCompact | `pre_compact_preserve.py` AAAK-compresses preservation block. Already working. |

#### Interactive (PM/Lead-Initiated via MCP)

| When | Who | What |
|------|-----|------|
| PM analyzing task | PM (Step 1) | `mcp__mempalace__mempalace_search(query=task, room="sprint_results")` → retrieve prior sprint outcomes |
| PM decomposing | PM (Step 2) | `mcp__mempalace__mempalace_kg_query(predicate="decided")` → what did we decide before? |
| Lead researching | Any lead | `mcp__mempalace__mempalace_search(query=topic, room="architecture")` → what's the project structure? |
| Post-sprint | PM (final) | `mcp__mempalace__mempalace_kg_add(subj, pred, obj)` → store major decisions explicitly |

### Token Budget for Memory Injection

| Context | Max Chars | ~Tokens | Where |
|---------|-----------|---------|-------|
| KG inject per lead (SubagentStart) | 1500 | 375 | Hook output `additionalContext` |
| Sprint context per lead (SubagentStart) | 2000 | 500 | Combined with KG |
| Prior sprint recall (PM MCP search) | 3 results | ~800 | PM reads interactively |
| KG decisions (PM MCP query) | 10 triples | ~200 | PM reads interactively |
| **Total memory overhead per lead** | | **~875** | |
...
global-hooks/hooks_SubagentStop/sprint_palace_store.py    ← lead result → palace + KG
...
data/mempalace.yaml                     ← add sprint_results + sprint_decisions rooms
...
Event types:
  ...
  mempalace_stored        kg_triple_written
  gate_unlocked           error
```
...
**Before decomposing, check what we already know** (via MCP tools — PM is Claude):

```
mcp__mempalace__mempalace_search(query=<user task>, wing=project, room="sprint_results", limit=3)
→ prior sprint outcomes for similar tasks

mcp__mempalace__mempalace_kg_query(predicate="decided", limit=10)
→ project-level decisions (e.g., "decided: use PostgreSQL")
```

Inject prior context into PM's planning (plain text, ≤800 tokens). **No AAAK here** — PM needs to read it.
...
3. **Store in mempalace** (automated via SubagentStop hook — AAAK-compressed for storage):
   - Palace: lead results → room=sprint_results
   - KG: decisions → triples (project, decided, X, valid_from=today)
4. **Archive** events.jsonl → `~/.claude/data/sprint_events/<id>.jsonl`
...
- **Sprint path**: inject into `additionalContext` (max 2000 chars total):
  ...
  4. KG decisions from `palace_init.get_project_kg()` (direct Python, not MCP)

#### `hooks_SubagentStop/sprint_palace_store.py`
- Only fires when `CAF_SPRINT_ROLE` env var is set
- **Reuses existing** `subagent_palace_store.py` logic — imports, doesn't duplicate
- AAAK-compresses lead result before palace storage (max 4000 chars stored)
  - Result stays plain text in IPC dir for TUI/PM reading
  - Only the palace copy is compressed (for space efficiency)
- Extracts decisions → KG triples (existing pattern)
- Emits sprint-event: `mempalace_stored`, `kg_triple_written`
- **Async** (`"async": true`) — non-blocking
...
```json
{
  ...
  "mempalace_writes": 4,
  "kg_triples_written": 7,
  "aaak_compressions": {"count": 4, "original_chars": 18200, "stored_chars": 820, "ratio": 22.2}
}
```
...
# AAAK storage savings
jq 'select(.type=="mempalace_stored") | {lead: .lead_role, original: .original_chars, stored: .stored_chars}' events.jsonl
...
3. Optionally: AAAK-compressed and stored in mempalace palace (room=sprint_events) for cross-sprint semantic search
4. KG triples persist indefinitely in `.mempalace/knowledge_graph.sqlite3`
...
| 9 | Create SubagentStop hook | hooks-builder | yes (imports existing) | `global-hooks/hooks_SubagentStop/sprint_palace_store.py` |
...
| 17 | Update mempalace.yaml | integration-builder | yes | `data/mempalace.yaml` |
...
- Plain text prompts ONLY (no AAAK on anything leads/PM need to read)
- Mempalace recall via MCP tools in Step 1
- Mempalace storage via hooks (automated, not in skill)
...
- Sprint path: inject ≤2000 chars (prompt + events + results + KG)
- Uses `palace_init.get_project_kg()` for KG (direct Python, not MCP)
- Validate: `uv run python -m py_compile` + returns `{}` when no sprint env

### 9. Create SubagentStop sprint hook
- **Task ID**: create-subagent-stop-hook
- **Depends On**: create-sprint-event
- **Assigned To**: hooks-builder
- **Agent Type**: builder
- **Parallel**: true
- Write `global-hooks/hooks_SubagentStop/sprint_palace_store.py` per Component 6 spec
- IMPORT from existing subagent_palace_store.py — do NOT duplicate
- AAAK-compress result before palace storage (result stays plain text in IPC)
- Extract decisions → KG triples
- Emit sprint-events for each storage action
- Async (non-blocking)
- Validate: `uv run python -m py_compile`
...
### 17. Update mempalace.yaml
- **Task ID**: update-mempalace
- **Depends On**: none
- **Assigned To**: integration-builder
- **Agent Type**: builder
- **Parallel**: true
- Add rooms: `sprint_results`, `sprint_decisions`, `sprint_events`, `research_results`
- Validate: YAML parses
...
### Memory
- [ ] SubagentStop hook stores lead results in mempalace palace (AAAK-compressed for storage)
- [ ] SubagentStop hook writes KG triples for decisions
- [ ] SubagentStart hook injects ≤2000 chars sprint context + KG decisions
- [ ] PM queries prior sprints via MCP on startup
- [ ] events.jsonl archived to `~/.claude/data/sprint_events/` on teardown
- [ ] AAAK only used for STORAGE, never for inter-agent communication
...
- [ ] Without mempalace: hooks skip storage gracefully (fail-open)
...
uv run python -m py_compile global-hooks/hooks_SubagentStop/sprint_palace_store.py
...
python3 -c "import yaml; yaml.safe_load(open('data/mempalace.yaml'))"
...
grep -q 'sprint_results' data/mempalace.yaml
...
### AAAK: Storage, Not Communication
AAAK output is **opaque encoded notation** (~45x compression, not human-readable). It is ONLY used for:
- **Storing** lead results in mempalace palace (compressed copy; original stays readable in IPC)
- **Archiving** events.jsonl for long-term cross-sprint queries
- **Pre-compact preservation** (existing production use)

It is NEVER used for: lead prompts, inter-lead messages, IPC files, TUI display, PM plans, or anything a human or Claude needs to read in real-time.

### Mempalace: Two Access Modes
1. **Hooks (automated)**: Direct Python import via `palace_init.py`. No LLM, no MCP, async, non-blocking. Used for automated storage on SubagentStop and KG injection on SubagentStart.
2. **Claude/Agents (interactive)**: 19 MCP tools. Used by PM for querying prior sprint results and making explicit KG writes.
...
| AAAK (storage only) | ~95% on archival | SubagentStop → palace |
...
- **mempalace** scales to GBs (ChromaDB + SQLite)
...
**Layer 6: mempalace is optional, not assumed**
All mempalace operations are fail-open. If not installed:
- No prior sprint recall (PM starts fresh — still functional)
- No long-term storage (results stay in /tmp/caf_sprint/ — still there for current session)
- No KG decisions (PM can't query past decisions — prompts are still written)
- AAAK compression returns original text unchanged

**What this means for installation**:
```bash
# Minimum viable install (any Claude Code user):
cd ~/.claude/skills && git clone <caf-repo>
# Sprint works with: agents-only mode, no gstack, no mempalace
