...
description: |
  ...
  design review context from /plan-design-review, Claude Design exports (HTML/zip
  handoff from claude.ai/design), or from scratch with a user description.
  ...
  "build me a page", "implement this design", "I exported from Claude Design",
  "Claude Design handed off to you", or after any planning skill.
  Proactively suggest when user has approved a design or has a plan ready. (gstack)
  Voice triggers (speech-to-text aliases): "build the design", "code the mockup", "make it real", "I exported from Claude Design", "Claude Design handed off", "implement the Claude Design export".
allowed-tools:
  - Bash
  - Read
  - Write
...
Also check if this session was initiated by a Claude Design handoff (claude.ai/design → "Handoff to Claude Code"). If the user's message contains a file path ending in `.html` or `.zip`, or mentions "Claude Design", treat that as a Claude Design export signal.
...
> E) I have a Claude Design export — let me provide the file path
...
If E: proceed to Case D below.

### Case D: Claude Design export (claude.ai/design handoff)

Trigger: user's message contains a file path to an `.html` or `.zip` file, mentions
"Claude Design", "handed off from Claude Design", or selected E above.

**Step D1: Locate the export file.**

If the user provided a path, use it directly. Otherwise run:

```bash
# Check Downloads for recent Claude Design exports (within last 24h)
setopt +o nomatch 2>/dev/null || true
ls -t ~/Downloads/*.html ~/Downloads/*.zip 2>/dev/null | head -5
ls -t /tmp/*.html /tmp/*.zip 2>/dev/null | head -5
```

If files found: show the list and use AskUserQuestion to confirm which one.
If no files found: use AskUserQuestion to ask the user for the file path.

**Step D2: Ingest the export.**

For `.html` export: read the file directly with the Read tool.
For `.zip` export:
```bash
_ZIPPATH="<user-provided-path>"
_EXTRACT_DIR="/tmp/claude-design-export-$$"
mkdir -p "$_EXTRACT_DIR"
unzip -q "$_ZIPPATH" -d "$_EXTRACT_DIR"
ls "$_EXTRACT_DIR"
```
Find the main HTML file in the extracted directory, then read it.

**Step D3: Parse the Claude Design HTML.**

Read the Claude Design export HTML. Extract:
- Visual structure: what screens/sections exist, layout type (grid, flex, stack)
- Color tokens: all CSS custom properties and hardcoded hex values
- Typography: font families, sizes, weights, line-heights
- Component inventory: buttons, cards, forms, nav, hero, etc.
- Any embedded design system notes or comments
- Responsive breakpoints already in the CSS

**Step D4: Reconcile with project's DESIGN.md.**

Read `DESIGN.md` if it exists. Project DESIGN.md tokens take priority over Claude Design
values for brand-level properties (primary colors, font family, spacing scale).
Claude Design values fill in the rest.

If there are significant conflicts (different font family, clashing palette), use
AskUserQuestion:
> Your project's DESIGN.md uses [X] but the Claude Design export uses [Y].
> Which should take priority for this implementation?
> A) Use DESIGN.md (keep your existing design system)
> B) Use Claude Design export values (adopt the new design)
> C) I'll tell you which properties to override

**Step D5: Set mode and screen name.**

Mode is `claude-design-import`. Ask the user for a screen name to use for the output
directory (e.g., "dashboard", "onboarding", "landing") unless it's obvious from the
Claude Design export filename or content.

Read `DESIGN.md` if it exists in the repo root. These tokens take priority for
system-level values (fonts, brand colors, spacing scale).

Proceed to Step 1 using the parsed Claude Design HTML as the visual reference.
In Step 1, use the extracted spec from Step D3 as the implementation spec instead of
running vision analysis — the HTML source is already the ground truth.

### Context summary

After routing, output a brief context summary:
- **Mode:** approved-mockup | plan-driven | freeform | evolve | claude-design-import
- **Visual reference:** path to approved PNG, Claude Design export HTML path, or "none"
- **CEO plan:** path or "none"
- **Design tokens:** "DESIGN.md" or "none"
- **Screen name:** from approved.json, Claude Design filename, user-provided, or inferred
