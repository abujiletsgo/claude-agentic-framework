...
1. Spawn the relevant domain consultant (no new user questions) — parallel with Gemini:
...
For Simple tasks (normally skip Wave 0a): on 1st QA failure, apply consultant self-healing — same pattern as Standard/Complex, but:
- Spawn ONE consultant (the most relevant domain — backend, frontend, or architecture)
