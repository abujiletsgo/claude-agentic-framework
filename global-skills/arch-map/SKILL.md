...
Grep patterns for reads/writes differ per language (Python, Node/TypeScript, Rust, Go) and mixed stacks require separate subgraphs with an explicit FFI boundary. See [references/tech-stack-patterns.md](references/tech-stack-patterns.md) for the full per-language pattern lists and mixed-stack guidance.

---

## Output File Template

`.claude/ARCHITECTURE.md` must open with a scannable ~15-line header (generation date, git hash, section index table) so future sessions can load just the header to orient themselves. The file ends with a regeneration footer. See [references/output-template.md](references/output-template.md) for the full ARCHITECTURE.md template.
...
Three worked examples (Data Science/ML, Next.js + Python API, CLI with Plugins) are in [references/tech-stack-patterns.md](references/tech-stack-patterns.md).
