...
Determine the error category (Syntax, Type, Runtime, Import/Module, Network, Permission, Configuration, Resource).
See [references/error-patterns.md](references/error-patterns.md) for the full error-category table and per-language patterns.
...
```bash
[ -f ~/.claude/skills/knowledge-db/scripts/knowledge_cli.py ] && \
  uv run ~/.claude/skills/knowledge-db/scripts/knowledge_cli.py search "<error-type>" --category error || true
```
...
[ -f ~/.claude/skills/knowledge-db/scripts/knowledge_cli.py ] && \
  uv run ~/.claude/skills/knowledge-db/scripts/knowledge_cli.py store \
    ...
    --tags "<language>,<framework>,<error-type>" || true
```
