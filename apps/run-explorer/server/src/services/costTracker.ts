 ...
 *
 * Ported from apps/observability/server/src/cost.ts — logic is identical.
 */
...
function filterByTime(entries: CostEntry[], since?: string, until?: string): CostEntry[] {
...
export async function getCostProjection(days: number = 7): Promise<CostProjection> {
  ...
  const avgDaily = activeDays.reduce((sum, d) => sum + d.total_cost, 0) / activeDays.length
